...
export default function Overview() {
  // Load both namespaces: overview (copy) + sessions (buttons/urls)
  const { t } = useTranslation(['overview', 'sessions']);
  ...
  return (
    <section id="overview" className="border-t border-white/10 bg-creme text-midnight-navy font-body">
      <div className="container mx-auto max-w-6xl px-6 sm:px-8 py-16 sm:py-20 space-y-12">
        ...
        <div className="max-w-3xl">
          ...
          <p className="text-base sm:text-lg leading-relaxed text-graphite">
            <Trans i18nKey="intro" ns="overview" />
          </p>
        </div>
        ...
        <div className="rounded-2xl bg-white/70 ring-1 ring-black/5 p-6 sm:p-8 shadow-sm space-y-6">
          ...
          <div className="grid md:grid-cols-2 grid-flow-row-dense gap-8">
            {order.map((i, idx) => {
              const steps = t(`oneToOne.blocks.${i}.steps`, { ns: 'overview', returnObjects: true });
              ...
              return (
                ...
                >
                  <p className="font-semibold">
                    <Trans i18nKey={`oneToOne.blocks.${i}.heading`} ns="overview" />
                  </p>
                  <p className="text-graphite">
                    <Trans i18nKey={`oneToOne.blocks.${i}.body`} ns="overview" />
                  </p>
                  {stepList.length ? (
                    <ul className="mt-2 ml-4 list-disc space-y-1 text-graphite">
                      {stepList.map((__, j) => (
                        <li key={j}>
                          <Trans i18nKey={`oneToOne.blocks.${i}.steps.${j}`} ns="overview" />
                        </li>
                      ))}
                    </ul>
                  ) : null}
          ...
          <div className="flex justify-end gap-3 pt-2">
            {urlInPerson ? (
              <ServiceButton
                url={urlInPerson}
                label={t('buttons.bookInPerson', { ns: 'sessions', defaultValue: 'Book 1:1 In-Person' })}
                color="caramel"
            ...
            {urlOnline ? (
              <ServiceButton
                url={urlOnline}
                label={t('buttons.bookOnline', { ns: 'sessions', defaultValue: 'Book 1:1 Online' })}
                color="caramel"
                className="whitespace-nowrap text-sm py-2"
              />
            ) : null}
        ...
        <div className="rounded-2xl bg-white/50 ring-1 ring-black/5 p-6 sm:p-8 space-y-5">
          <div className="flex items-center justify-between flex-wrap gap-3">
            <h3 className="font-display font-bold text-xl sm:text-2xl">
              {t('groups.title', { ns: 'overview' })}
            </h3>
            <span className="inline-flex items-center rounded-full px-3 py-1 text-sm ring-1 ring-black/10 bg-white">
              {t('groups.badge', { ns: 'overview' })}
            </span>
          </div>

          <p className="text-graphite leading-relaxed">
            <Trans i18nKey="groups.blurb" ns="overview" />
          </p>
          ...
          <ul className="grid sm:grid-cols-2 gap-2">
            {groupBullets.map((_, i) => (
              ...
              >
                ...
                <p className="text-graphite">
                  <Trans i18nKey={`groups.bullets.${i}`} ns="overview" />
                </p>
              </motion.li>
            ))}
          </ul>
          ...
          <div className="flex justify-end pt-2">
            {urlGroup ? (
              <ServiceButton
                url={urlGroup}
                label={t('buttons.bookGroup', { ns: 'sessions', defaultValue: 'Book Group' })}
                color="caramel"
                className="whitespace-nowrap text-sm py-2"
              />
            ) : null}
        ...
        <p className="text-sm text-graphite/80">
          <Trans i18nKey="footnote" ns="overview" />
        </p>
      </div>
    </section>
  );
